...
**Standalone phone (`jarvis_telefon.py` + `Jarvis-Telefon.bat`)**: this runs the Linphone line next to any Jarvis folder without touching its code. It needs `core/sip_phone.py` copied next to it as `sip_phone.py`, so `sip_phone.py` must stay free of other `core` imports. It talks to Gemini over the OpenAI-compatible endpoint, or to Ollama (`LLM_PROVIDER`/`OLLAMA_*` in `.env`) using stdlib `urllib`. It accepts `POST http://127.0.0.1:8431/call` from other programs. Typing `ara` in its window starts a call, and so does `hotword_loop`, which listens on the PC mic (VAD, then Google STT) for "call me" or "beni ara" and skips listening while a call is active (`JARVIS_PHONE_HOTWORD=0` disables it). `jarvis_telefon_patch.py` adds a "call me" / "beni ara" rule to the owner's other Jarvis project, specifically to `backend/fast_path.py` `match()`, and that rule POSTs to this control address. `JARVIS_PHONE_LANG` (default `en-US`) sets the STT, prompt and in-call phrases (`sip_phone.TEXTS`). If `ELEVENLABS_API_KEY` and `ELEVENLABS_VOICE_ID` are set, it speaks through ElevenLabs `ulaw_8000`. When PyAV and Pillow are installed, `VideoReceiver` depacketizes VP8 from a video call (receive only, and it sends RTCP PLI for keyframes). The latest camera frame goes to Gemini as an `image_url` part, while the SDP answer mirrors the offer's m-lines.
