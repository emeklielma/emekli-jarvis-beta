...
# A list of models to cycle through if we hit rate limits!
MODELS = [
...
def generate_response(prompt):
    ...
    if current_time - last_request_time < 5:
        return "Please wait a few seconds before speaking again so my circuits don't overheat."
    ...
    try:
        ...
        while attempts < len(MODELS):
            response = requests.post(get_url(), headers=headers, json=payload, timeout=10)
            ...
            if response.status_code == 200:
                ...
                if 'functionCall' in part:
                    ...
                    resp2 = requests.post(get_url(), headers=headers, json=payload, timeout=10)
                    
                    if resp2.status_code == 200:
                        data2 = resp2.json()
                        ai_text = data2['candidates'][0]['content']['parts'][0].get('text', "Got it. Action completed successfully.")
                ...
                # Rate limit hit! Switch to the next model in the list
                print(f"Model {MODELS[current_model_idx]} hit rate limit. Switching model...")
